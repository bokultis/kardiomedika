<?php
/**
 * Ajax Dialog Form action helper
 * 
 * @package    HCMS
 ...
 */
class HCMS_Controller_Action_Helper_AjaxDialogForm extends Zend_Controller_Action_Helper_Abstract
{
    const MSG_OK = "ok";
    const MSG_ERR = "err";
    const MSG_INFO = "info";
    ...
    public function init(){
        if(isset ($this->_actionController) && $this->_actionController->getRequest()->isXmlHttpRequest()) {
            ...
            if(isset ($this->_actionController->view)){
                $this->_actionController->view->headScript()->appendFile('/js/ajaxDialogForm.js');
            }
        }
    }
