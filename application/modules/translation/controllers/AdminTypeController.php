 ...
 * @author marko
 */
class Translation_AdminTypeController extends HCMS_Controller_Action_Admin
{
    /**
     *
    ...
    public function listAction() {
        ...
        $records = Translation_Model_TypeMapper::getInstance()->fetchAll($criteria, $order, $paging);
        $data = array(
            'total' => $paging['total'],
            'page' => $paging['page'],
    ...
    public function editAction(){
        ...
        //create form object
        $form = new Translation_Form_Type($data);
        ...
        if ($this->_formHelper->isSave()) {            
            ...
            if($form->isValid()) {                
                ...
                $type = new Translation_Model_Type($values);                
                Translation_Model_TypeMapper::getInstance()->save($type);                
                //save done, return success
                return $this->_formHelper->returnSuccess($this->view->url(array('action' => 'index')), $this->translate('Type saved.'));
            }
            else {
        ...
        elseif(!$this->_formHelper->getRequest()->isPost()) {
            //edit action
            if(isset ($id) && $id > 0) {
                $type = new Translation_Model_Type();
                if(!Translation_Model_TypeMapper::getInstance()->find($id, $type)){
