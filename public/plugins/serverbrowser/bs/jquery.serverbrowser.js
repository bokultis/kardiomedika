    ...
    $.fn.serverbrowser = function(options)
    {
        ...
        return this.each(function()
        {
            ...
            $.loadTpl(opts.template,function(tplStr){
                ...
                //UP
                self.find(".sbToolbar button.up").click(function(){
                    enterDir('..');
                    $(this).blur();
                    return false;
                });
                ...
                //NEW
                self.find(".sbToolbar button.new").click(function(){
                    $(this).blur();
                    $.fn.promptBS({
                        title: _("Prompt"),
                ...
                //RENAME
                self.find(".sbToolbar button.rename").click(function(){
                    $(this).blur();
                ...
                //DELETE
                self.find(".sbToolbar button.delete").click(function(){
                    $(this).blur();
                    var selectedObj = getSelected();
                    if(!selectedObj){
                        $.flashMessenger(_("Please select a file or a directory!"),{clsName:"warn"});
                ...
                //UPLOAD
                self.find(".sbToolbar button.upload").click(function(){
                    $(this).blur();
        ...
        function updateStats(){
            ...
            json.stats({
                'success':function(data){
                    ...
                    //self.find('#sbExtList .extList').text(data.extensions.join(", ")); // XXX take care about this
                },
                'error':function (self,req,stat,err,id,key){
                    alert(err);
                }
        ...
        function updateBreadCrumbs(){
            ...
            var currDir = "/";
            var html = '<ol class="breadcrumb"><li data-path="' + currDir + '"><a href="#">Start</a></li>';
            for(var i = 0; i < parts.length; i++){
                ...
                currDir += parts[i];
                html += '<li data-path="' + currDir + '"><a href="#">' + parts[i] + '</a></li>';
            }
            html += '</ol>';
            self.find('.sbBreadcrumb').html(html);
            self.find('.sbBreadcrumb li a').click(function(){
        ...
        function updateFileList(data,selectFile){
            ...
            for(var file in data){
                if(data[file].type == 'dir'){
                    ...
                    fileSize = '<span class="badge">' +  $.formatFileSize(data[file].size) + '</span>'

                }
                var fullName = ltrim(file,'./');
                var fileNameHtml = '<span class="glyphicon glyphicon-' + glyphicon + '"></span> <abbr title="' + fullName + '">' + fullName  + '</abbr>';
                items += '<li class="list-group-item ' + className + '" data-path="' + htmlspecialchars(file) + '">' + fileNameHtml + fileSize + '</li>';
            }
            ...
            self.find('.sbList li').each(function(){
                ...
                $(this).click(function(){
                    if(opts.selectableOptions.unselected){
                        opts.selectableOptions.unselected(null,self.find('.sbList li.active'));
                    }
                    self.find('.sbList li').removeClass("active");
        ...
        function getSelectedObj(){
            var selObject = self.find(".sbList li.active");
        ...
        function getUploadDialog(){
            if(uploadDialog == null){
                ...
                //buttons
                uploadDialog.find('#uploadButton').click(function() {
                    //trigger submit
                ...
                $(uploadDialog).find('form').submit(function(){
                    return AIM.submit(this,{
                        'onStart' : function(){
//                            ajaxLoader.show(); // deprecated !!!
                            $(".error").remove();
                            return true;
                        },
                        'onComplete' : function(response){
//                            ajaxLoader.hide(); // deprecated !!!
        ...
        function copyPaste(action, path, type) {
            ...
            case "paste":
                ...
                $.ajax({
                  type: "POST",
                  url: sessionUrl("/" + CURR_LANG + "/file-server/paste/active_module/" + opts.activeModule),
    ...
    {
        'template':'/plugins/serverbrowser/bs/template.tpl',
